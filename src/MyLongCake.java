...
public class MyLongCake {
   public int cut(int n) {
	...
	for(int i = 1; i <= n/2; i++){
            if(n % i == 0){ //assumin i friends arrive
                int chunks = n / i; // then total cuts will be CHUNKS, but if that chunk is already cut, no need to cut again
                for(int k = chunks; k <= n; k += chunks)
                    if(!cut[k]){
                        cut[k] = true;
                        cutHere++;
                    }
